...
public class UserController extends AbstractController {
    ...
    public ModelAndView findUserById(ModelAndView modelAndView, User user, Integer pageNumber,
        ...
        page.setSize(6);
        // 调用分页查询方法！!
        IPage<User> userIPage = iUserService.selectPage(page, wrapper);
        // 存放一个数组用来让foreach遍历
    ...
    @GetMapping(value = "/findUserById2")
    public ModelAndView findUserById2(ModelAndView modelAndView, User user, Integer pageNumber,
                                      Page page, HttpServletRequest request) {
        if (request.getSession().getAttribute("result") != null) {
            request.getSession().setAttribute("result", "");
        }
        ...
        modelAndView.setViewName("user/caiwu_user_list");
        return modelAndView;
    }
    ...
    public String addEditUser(User user, HttpServletRequest request) {
        User user1 = iUserService.findById(user);
